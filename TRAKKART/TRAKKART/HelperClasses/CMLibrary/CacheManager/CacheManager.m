...
//        NSLog(@"Database Already Exists!!");
	...
	NSString *defaultDBPath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:CacheManagerSqliteName];
...
{
    if(![APPUtility checkIfStringContainsText:key])
        return nil;
    ...
    {
        if(errMsg && [APPUtility checkIfStringContainsText:[NSString stringWithUTF8String:errMsg]] && errMsg != [@"(null)" UTF8String])
            sqlite3_free(&errMsg);
    }
    else
    {
        if (sqlite3_step(statement) == SQLITE_ROW)
        {
            ...
            [cache setCacheKey:[APPUtility getStringFromChar:(char *) sqlite3_column_text(statement, ++colIndex)]];
            ...
            [cache setCreatedDate:[self getDateObjectFromString:[APPUtility getStringFromChar:(char *) sqlite3_column_text(statement, ++colIndex)]]];
            [cache setModifiedDate:[self getDateObjectFromString:[APPUtility getStringFromChar:(char *) sqlite3_column_text(statement, ++colIndex)]]];
        }
    }
...
-(BOOL)cacheData:(id)data forKey:(NSString *)key
{
    ...
    if (sqlite3_step(statement) != SQLITE_DONE) {
        // error handling...
        NSLog(@"%@",[APPUtility getStringFromChar:sqlite3_errmsg(dataBaseConnection)]);
    }
    else
...
-(BOOL)clearDataForKey:(NSString *)key
{
    ...
    if(sqlite3_exec(dataBaseConnection, sqlQuery, NULL, NULL, &errMsg) != SQLITE_OK) {
        NSLog(@"Error processing Query. Reason :- %s",errMsg);
        if(!errMsg || [APPUtility checkIfStringContainsText:[NSString stringWithUTF8String:errMsg]])
            sqlite3_free(&errMsg);
        success=NO;
    } else {
...
//    BOOL success=NO;
    ...
    if(!data || ![APPUtility checkIfStringContainsText:key] || ![APPUtility checkIfStringContainsText:extension])
        return NO;
